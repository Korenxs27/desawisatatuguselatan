...
import { 
  ...
  Upload, X, Download, ArrowRight, Copy, Check, MessageSquare, Calendar, Users, Percent 
} from "lucide-react";
import toast, { Toaster } from "react-hot-toast";

interface TourOrderFormProps {
  productId: number;
  productName: string;
  productPrice: number;
  stockStatus?: string;
  minPeserta: number;
  diskonMinimalPeserta: number;
...
export default function ClientBookingForm({ 
  ...
  const [participants, setParticipants] = useState(minPeserta || 1);
  ...
  const isDiscountActive = diskonMinimalPeserta > 0 && diskonNominal > 0 && participants >= diskonMinimalPeserta;
  const finalPricePerPerson = isDiscountActive ? Math.max(0, productPrice - diskonNominal) : productPrice;
  const totalPrice = finalPricePerPerson * participants;

  useEffect(() => {
    setMounted(true);
    
    // 1. CEK STATUS LOGIN & AUTO-FILL DATA AKUN
    ...
    // Default tanggal kunjungan: Besok
    ...
    // 3. FETCH METODE PEMBAYARAN REKENING & QRIS
  ...
  const handleBooking = async (e: React.FormEvent) => {
    ...
    if (participants < minPeserta) {
      toast.error(`Minimal pemesanan untuk paket ini adalah ${minPeserta} peserta.`);
      return;
    }
    ...
    const loadToast = toast.loading("Memproses reservasi & jadwal kunjungan...");
    ...
    try {
      ...
      if (result.success && result.order_id) {
        ...
        toast.success("Reservasi berhasil dibuat, silakan lakukan transfer.");
      } else {
        toast.error(`Gagal membuat booking: ${result.message || "Kesalahan server."}`);
      }
    } catch (error) {
      toast.dismiss(loadToast);
      toast.error("Terjadi kendala jaringan saat memproses booking.");
    } finally {
  ...
  const handleUploadBukti = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setInvoiceData({
        ...
        bukti_url: buktiUrl,
        jenis_pesanan: "Paket Wisata Tugu Selatan",
        tgl_kunjungan: visitDate,
      });
    } catch (err) {
      toast.dismiss(loadToast);
  ...
  const handleSendToWhatsApp = () => {
    ...
    const finalName = name || invoiceData?.customer_name || "Pemesan Paket";
    const finalPhone = phone || invoiceData?.customer_phone || "-";
    const finalBuktiUrl = uploadedBuktiUrl || invoiceData?.bukti_url || "";

    const textMessage = 
`Halo Admin Desa Wisata Tugu Selatan, saya ingin mengkonfirmasi pembayaran booking paket wisata.

*ID Order:* #${finalOrderId}
*Nama Pemesan:* ${finalName}
*Kategori:* Paket Wisata Tugu Selatan
    ...
    <>
      ...
      <form onSubmit={handleBooking} className="space-y-4">
        <div className="space-y-1">
          ...
          <input 
            type="text" required placeholder="Nama lengkap ketua / perwakilan" value={name} 
        ...
        <div className="space-y-1 p-3 bg-emerald-50/50 rounded-2xl border border-emerald-100">
          <label className="text-[10px] uppercase font-bold text-emerald-700 flex items-center gap-1">
            <Calendar size={12}/> Jadwal Rencana Tanggal Kunjungan
          </label>
          <input 
            type="date" required value={visitDate} 
            min={new Date().toISOString().split("T")[0]} 
            onChange={(e) => setVisitDate(e.target.value)} 
            className="w-full bg-white border border-emerald-200 p-3 rounded-xl text-xs focus:outline-none focus:border-emerald-500 text-neutral-800 font-medium" 
          />
        </div>

        <div className="space-y-1">
          <label className="text-[10px] uppercase font-bold text-neutral-400 flex items-center gap-1">
            <Users size={12}/> Jumlah Peserta (Min. {minPeserta} orang)
          </label>
          <input 
            type="number" required min={minPeserta} value={participants} 
            onChange={(e) => setParticipants(Math.max(minPeserta, parseInt(e.target.value) || minPeserta))} 
            className="w-full bg-white border border-neutral-200 p-3 rounded-xl text-xs focus:outline-none focus:border-emerald-500 text-neutral-800" 
          />
        </div>

        {diskonMinimalPeserta > 0 && diskonNominal > 0 && (
          <div className={`p-3 rounded-xl border text-[11px] flex items-center gap-2 ${isDiscountActive ? 'bg-emerald-50 border-emerald-200 text-emerald-800 font-medium' : 'bg-neutral-50 border-neutral-200 text-neutral-500'}`}>
            <Percent size={14} className={isDiscountActive ? "text-emerald-600 shrink-0" : "shrink-0"} />
            <span>
              {isDiscountActive 
                ? `Selamat! Anda Mendapat Diskon Rombongan Rp ${diskonNominal.toLocaleString("id-ID")} / orang.` 
                : `Pesan minimal ${diskonMinimalPeserta} orang untuk mendapatkan potongan harga Rp ${diskonNominal.toLocaleString("id-ID")} / orang.`}
            </span>
          </div>
        )}
              ...
              {paymentMethods.map((method) => (
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <input 
                      type="radio" name="payment_method_tour" 
        ...
        <div className="bg-neutral-50 p-4 rounded-2xl border border-neutral-100 text-xs text-neutral-500 space-y-1.5">
          <div className="flex justify-between">
            ...
            <span className={isDiscountActive ? "line-through text-neutral-400" : ""}>Rp {productPrice.toLocaleString("id-ID")}</span>
          </div>
          
          {isDiscountActive && (
            <div className="flex justify-between text-emerald-600 font-medium">
              <span>Harga setelah diskon rombongan</span>
              <span>Rp {finalPricePerPerson.toLocaleString("id-ID")}</span>
            </div>
          )}
        ...
        >
          <ShoppingBag size={14}/> {isProcessing ? "Memproses Booking..." : "Booking & Lanjut Pembayaran"}
        </button>
      </form>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-md p-4 overflow-y-auto">
          <div className="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl space-y-4 relative my-auto max-h-[90vh] overflow-y-auto">
            ...
            <div className="text-center space-y-1">
              <span className="px-3 py-1 bg-emerald-50 text-emerald-700 text-[10px] font-bold rounded-full">Selesaikan Pembayaran Paket</span>
            ...
            <div className="p-6 sm:p-10 overflow-y-auto space-y-6 bg-white text-neutral-800 flex-1">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-4">
                <div>
                  ...
                  <p className="text-xs text-neutral-500">Paket Wisata & Petualangan Puncak</p>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-xs">
                <div className="space-y-1">
                  ...
                  <p className="text-neutral-600">Jadwal Kunjungan: <strong className="text-neutral-800">{invoiceData.tgl_kunjungan}</strong></p>
                </div>
                ...
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody className="divide-y divide-neutral-100">
                    <tr>
                      ...
                      <td className="p-2.5 text-center text-neutral-600">{invoiceData.quantity} Orang</td>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 pt-2 border-t">
                <div className="text-[11px] text-neutral-500 max-w-xs space-y-1">
                  ...
                  <p>Terima kasih telah memesan paket wisata di Desa Wisata Tugu Selatan. Konfirmasi WhatsApp Anda telah dikirimkan ke admin.</p>
                </div>
              ...
              <div className="pt-4 flex justify-between items-end text-xs text-neutral-500 border-t">
                ...
                <div className="text-center space-y-6">
                  ...
                  <p className="font-bold text-neutral-800 underline">Pengelola BUMDes</p>
                </div>
              </div>
