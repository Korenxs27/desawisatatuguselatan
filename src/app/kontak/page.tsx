...
export default function KontakDanMitigasiPage() {
  ...
  useEffect(() => {
    async function fetchBmkgWeather() {
      try {
        ...
        // Struktur data BMKG mengembalikan array data cuaca per jam/hari
        if (json && json.data && json.data.length > 0) {
          // Mengambil cuaca terdekat dari jam saat ini (index pertama cuaca)
        ...
        // Fallback jika terjadi kendala jaringan / CORS pada client-side fetch
        setWeatherData({
          suhu: "19°C",
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-emerald-50/40 to-teal-50/50 pt-10 sm:pt-10 pb-24 px-4 sm:px-6 font-sans text-slate-800 relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto space-y-12 relative z-10">
        ...
        <div className="bg-white/80 backdrop-blur-2xl p-6 sm:p-10 rounded-[2.5rem] border border-white/90 shadow-xl shadow-slate-200/50 space-y-8">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200/80 pb-6">
            <div className="flex items-center gap-3.5">
              <div className="w-12 h-12 bg-emerald-100 text-emerald-600 rounded-2xl flex items-center justify-center shadow-inner shrink-0">
                <CloudSun size={24} />
              </div>
              <div>
                <h2 className="text-base sm:text-lg font-bold text-slate-900">Prakiraan Cuaca Resmi BMKG</h2>
                <p className="text-[11px] text-slate-500">Kode Wilayah: 32.01.25.2006 (Tugu Selatan, Cisarua, Bogor)</p>
              </div>
            </div>
            
            <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 px-4 py-2 rounded-2xl shadow-sm">
              <span className={`w-2.5 h-2.5 rounded-full ${weatherData.statusWarna} animate-pulse`}></span>
              <span className="text-[11px] font-bold text-emerald-800">Status: {weatherData.statusSiaga}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
            ...
            <div className="bg-slate-50/90 border border-slate-200/80 p-5 rounded-2xl space-y-1.5 shadow-sm hover:border-emerald-500/30 transition">
          ...
          <div className="bg-gradient-to-br from-slate-900 to-emerald-950 text-white p-6 sm:p-8 rounded-3xl space-y-4 shadow-xl">
            <div className="flex items-center gap-2 text-emerald-400 font-bold text-xs uppercase tracking-wider">
              <ShieldCheck size={18} /> Prosedur Tetap Mitigasi Bencana BPH Tugu Selatan
            </div>
        ...
        <div className="bg-white/80 backdrop-blur-2xl p-6 sm:p-8 rounded-[2.5rem] border border-white/90 shadow-xl shadow-slate-200/50 space-y-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
            <div>
              ...
              <h3 className="text-xl font-bold text-slate-900">Peta Kampung Koboy (Tugu Selatan)</h3>
            </div>
            <a 
          ...
          <div className="relative w-full h-[350px] sm:h-[400px] rounded-3xl overflow-hidden border border-slate-200/80 shadow-inner bg-slate-100">
            <iframe 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
          ...
          <div className="bg-white/80 backdrop-blur-2xl p-8 rounded-[2.5rem] border border-white/90 shadow-xl shadow-slate-200/50 space-y-8 flex flex-col justify-between">
            <div className="space-y-6">
              <div className="space-y-2">
                <span className="text-xs font-bold text-emerald-600 uppercase tracking-widest">Kantor Sekretariat</span>
                <h2 className="text-2xl font-bold text-slate-900">Badan Pengurus Harian (BPH)</h2>
              ...
              <div className="space-y-3.5">
                <div className="flex items-start gap-3.5 bg-slate-50/90 p-4 rounded-2xl border border-slate-200/70">
          ...
          <div className="bg-white/90 backdrop-blur-2xl p-8 rounded-[2.5rem] border border-white/90 shadow-2xl flex flex-col justify-between">
            <div className="space-y-2 mb-6">
              <span className="text-xs font-bold text-emerald-600 uppercase tracking-widest">Layanan Interaktif</span>
              <h3 className="text-2xl font-bold text-slate-900">Kirim Pesan / Pengaduan</h3>
            ...
            {!isSubmitted ? (
              ...
              <div className="bg-emerald-50 border border-emerald-200 p-8 rounded-3xl text-center space-y-4 my-auto">
                ...
                <button onClick={() => { setIsSubmitted(false); setFormData({ name: "", phone: "", message: "" }); }} className="bg-slate-900 hover:bg-emerald-900 text-white px-6 py-2.5 rounded-xl text-xs font-bold transition">
                  Kirim Pesan Lainnya
                </button>
              </div>
            )}
